...
export function loadScene(_scene) {
  ...
  if (scene == 0) {
    ...
    for (let i = 500; i--; ) GameObjects.spawnRandomObject(vec2(LJS.rand(1, 39), LJS.rand(10, 50)));
  }
  if (scene == 1) {
    sceneName = 'Pyramid';
  ...
  if (scene == 2) {
    ...
    GameObjects.spawnBox(vec2(24, 8), vec2(34, 1), LJS.randColor(), LJS.box2d.bodyTypeStatic, false, -0.15);
  }
  if (scene == 3) {
    sceneName = 'Car';
  ...
  if (scene == 5) {
    ...
    GameObjects.spawnRandomEdges();
    for (let i = 100; i--; )
      GameObjects.spawnRandomObject(
        vec2(LJS.rand(1, 39), LJS.rand(20)),
        2,
  ...
  if (scene == 6) {
    sceneName = 'Joints';
    {
      ...
      const l1 = new LJS.EngineObject(vec2(20, 8), vec2(39, 0.5), 0, 0, LJS.GRAY);
      const l2 = new LJS.EngineObject(vec2(20, 50), vec2(0.5, 99), 0, 0, LJS.GRAY);
    ...
    }
    {
      ...
      const line = new LJS.EngineObject(vec2(20, 15), vec2(10, 0.2), 0, 0, LJS.BLACK);
      line.gravityScale = 0;
      line.renderOrder = -1;
    }
    {
  ...
  if (scene == 7) {
    ...
    for (let i = 200; i--; ) GameObjects.spawnRandomObject(vec2(LJS.rand(1, 39), LJS.rand(10, 50)));
  }
  if (scene == 8) {
    sceneName = 'Mobile';
