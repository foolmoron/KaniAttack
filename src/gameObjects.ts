...
export function spawnBox(
  ...
  size = 1,
  color = LJS.WHITE,
  type = LJS.box2d.bodyTypeDynamic,
  applyTexture = false,
  angle = 0
) {
...
export function spawnRandomObject(pos, scale = 1, type = LJS.box2d.bodyTypeDynamic, angle = 0) {
  if (LJS.randInt(2)) {
    const size = vec2(scale * LJS.rand(0.5, 1), scale * LJS.rand(0.5, 1));
    return spawnBox(pos, size, LJS.randColor(), type, true, angle);
  } else {
    const diameter = scale * LJS.rand(0.5, 1);
...
export function spawnPyramid(pos, count) {
  for (let i = count + 1; i--; )
    for (let j = i; j--; ) spawnBox(pos.add(vec2(j - i / 2 + 0.5, count - i + 0.5)), 1, LJS.randColor());
}

export function spawnDominoes(pos, count, size = vec2(0.5, 3)) {
...
export class PulleyJointObjects extends LJS.Box2dObject {
  ...
  render() {
    ...
    LJS.drawLine(topPos, this.connectionPos, 0.2, LJS.BLACK);
    super.render();
  }
}
...
export class SoftBodyObject extends LJS.Box2dObject {
  ...
  getNode(x, y) {
    if (vec2(x, y).arrayCheck(this.sizeCount)) return this.nodes[y * this.sizeCount.x + x];
  }
  destroy() {
  ...
  constructor(pos, scale, sizeCount, color, maxJointStress = 10) {
    ...
    for (let y = sizeCount.y; y--; )
      for (let x = sizeCount.y; x--; ) {
        ...
        const center = vec2(x - nodeSize.x / 2, y - nodeSize.y / 2);
        const p = pos.add(center.multiply(spacing));
        const o = new LJS.Box2dObject(p, vec2(0.4), Game.spriteAtlas.circle, 0, color);
        o.addCircle(objectDiameter);
        o.setFilterData(2, 2);
...
export class RagdollObject extends LJS.Box2dObject {
  constructor(pos, scale = 1, color = LJS.randColor()) {
    ...
    const addLimb = (offset, size, isCircle = false) => {
      ...
      if (isCircle) o.addCircle(size.x, vec2(), density, friction, restitution);
      else o.addBox(size, vec2(), 0, density, friction, restitution);
      this.limbs.push(o);
      return o;
    };
    ...
    // head
    const head = addLimb(vec2(0, torsoHeight / 2 + headSize / 2 + 0.1 * scale), vec2(headSize), true);
    addJoint(this, head, pos.add(vec2(0, torsoHeight / 2)), -0.5, 0.5);

    // arms
    const shoulderY = torsoHeight / 2 - 0.2 * scale;
    for (const side of [-1, 1]) {
      const shoulder = pos.add(vec2((side * torsoWidth) / 2, shoulderY));
      const upperArm = addLimb(
        vec2(side * (torsoWidth / 2 + upperArmLen / 2), shoulderY),
        vec2(upperArmLen, limbWidth)
      );
      addJoint(this, upperArm, shoulder, -2, 2);

      const elbow = shoulder.add(vec2(side * upperArmLen, 0));
      const lowerArm = addLimb(
        vec2(side * (torsoWidth / 2 + upperArmLen + lowerArmLen / 2), shoulderY),
        vec2(lowerArmLen, limbWidth)
      );
    ...
    for (const side of [-1, 1]) {
      ...
      const upperLeg = addLimb(
        vec2((side * torsoWidth) / 4, -torsoHeight / 2 - upperLegLen / 2),
        vec2(limbWidth, upperLegLen)
      );
      addJoint(this, upperLeg, hip, -0.5, 2.5);

      const knee = hip.add(vec2(0, -upperLegLen));
      const lowerLeg = addLimb(
        vec2((side * torsoWidth) / 4, -torsoHeight / 2 - upperLegLen - lowerLegLen / 2),
        vec2(limbWidth, lowerLegLen)
      );
...
const sound_explosion = new LJS.Sound([0.5, 0.2, 72, 0.01, 0.01, 0.2, 4, , , , , , , 1, , 0.5, 0.1, 0.5, 0.02]);
...
export function explosion(pos, radius = 3, strength = 300) {
  ...
  new LJS.ParticleEmitter(
    ...
    Game.spriteAtlas.dot, // tileInfo
    hsl(0, 0.8, 0.5),
    hsl(0.15, 0.8, 0.5), // colorStartA, colorStartB
    hsl(0, 1, 0.5, 0),
