...
randBool((chance = 0.5)); // Random boolean with given chance (0 to 1)
...
Color.mutate((amount = 0.05), (alphaAmount = 0)); // Randomly diverge from this color
...
drawLine(posA, posB, (width = 0.1), (color = WHITE), (pos = (0, 0)), (angle = 0));
drawLineList(points, (width = 0.1), color, (wrap = false), (pos = (0, 0)), (angle = 0));
...
tileDefaultBleed = 0.3; // How much smaller to draw tiles to prevent bleeding
```
...
objectDefaultFriction = 0.8; // How much to slow when touching (0-1)
...
debugLine(posA, posB, color, (width = 0.1), time); // Draw debug line
...
debugPointSize = 0.5; // Size to render debug points by default
